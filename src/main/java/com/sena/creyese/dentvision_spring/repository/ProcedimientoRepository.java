...
public interface ProcedimientoRepository extends JpaRepository<Procedimiento,Long> {
    ...
    @Query("SELECT p FROM Procedimiento p WHERE p.descripcion LIKE %:nombre%")
    ...
    @Query("SELECT p FROM Procedimiento p WHERE p.estado = true ORDER BY p.descripcion ASC")
    ...
    @Query("SELECT p FROM Procedimiento p WHERE p.descripcion LIKE %:keyword% OR p.descripcion LIKE %:keyword%")
